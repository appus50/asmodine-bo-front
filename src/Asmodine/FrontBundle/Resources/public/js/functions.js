...
function createExportLoader(title){
    if(jQuery('#exportLoader').length > 0) return;

    var html = '<div id="exportLoader" style="text-align: center; width:190px;"><span class="title">' +title+'</span>'+
        '<div class="progress progress-striped active" style="margin: 10px 0 5px 0;">' +
        '<div class="progress-bar" style="width: 0%" aria-valuemax="100" aria-valuemin="0" aria-valuenow="0" role="progressbar">' +
        '<span  class="sr-only percent">0% </span>' +
        '</div>' +
        '</div>' +
        '<span class="rowsProcessed">0</span>/<span class="rowsTotal">0</span> &nbsp;&nbsp;<span class="percent"></span>' +
        '</div>';

    notify(html,'inverse',0);
}
function updateExportLoader(rowsProcessed,rowsTotal,title){
    if(jQuery('#exportLoader').length == 0){
        createExportLoader(title);
    }
    var percent = parseInt((rowsProcessed/rowsTotal)*100);
    jQuery('#exportLoader').find('.title').html(title);
    jQuery('#exportLoader').find('.percent').html(percent+'%');
    jQuery('#exportLoader').find('.rowsProcessed').html(rowsProcessed);
    jQuery('#exportLoader').find('.rowsTotal').html(rowsTotal);
    jQuery('#exportLoader').find('.progress-bar').css('width',percent+'%');

}
function removeExportLoader(){
    if(jQuery('#exportLoader').length > 0){
        jQuery('#exportLoader').closest('.alert').remove();
        jQuery('#exportBtn,#importBtn').show();
		jQuery('.editProductLink').show();
    }
}


function createImportLoader(title){
    if(jQuery('#importLoader').length > 0) return;

    var html = '<div id="importLoader" style="text-align: center; width:190px;"><div><span class="title">' +title+'</span></div><div>'+
        '<div class="preloader pls-amber">        <svg viewBox="25 25 50 50" class="pl-circular">        <circle r="20" cy="50" cx="50" class="plc-path"/>        </svg>        </div>' +
        '</div><div><span class="rowsProcessed">0</span></div>' +
        '</div>';

    notify(html,'inverse',0);
}
function updateImportLoader(rowsProcessed,title){
    if(jQuery('#importLoader').length == 0){
        createImportLoader(title);
    }
    jQuery('#importLoader').find('.title').html(title);
    jQuery('#importLoader').find('.rowsProcessed').html(rowsProcessed);

}
function removeImportLoader(){
    if(jQuery('#importLoader').length > 0){
        jQuery('#importLoader').closest('.alert').remove();
        jQuery('#exportBtn,#importBtn').show();
		jQuery('.editProductLink').show();
    }
}
...
jQuery(document).ready(function(){
    if(jQuery('#cgu-beta').length > 0){
        if (jQuery("#cgu-beta").checked) {
            jQuery("#cgu-beta").addClass('cgu-ok');
        }else{
            jQuery("#cgu-beta").addClass('cgu-ko')
        }
        jQuery("#cgu-beta").on("click", function(e) {
            ...
            if(checked) {
                jQuery(this).removeClass('cgu-ok').addClass('cgu-ko');
            }else{
                jQuery(this).addClass('cgu-ok').removeClass('cgu-ko');
            }
        });
    }
    ...
    (function () {
        ...
        jQuery('body').on('keyup','.filter-url',function(){
            jQuery(this).val(slugify(jQuery(this).val()));
        });


        jQuery('body').on('keyup change blur','.slugField',function(){
	    ...
	    jQuery('.profile-menu .main-menu').hide();
            ...
            if (jQuery('#header').hasClass('sidebar-toggled')) {
                jQuery(document).on('click', function (e) {
                    if ((jQuery(e.target).closest($elem).length === 0) && (jQuery(e.target).closest($elem2).length === 0)) {
                        setTimeout(function(){
                            jQuery($elem).removeClass('toggled');
                            jQuery('#header').removeClass('sidebar-toggled');
                            jQuery($elem2).removeClass('open');
                        });
                    }
                });
            }
